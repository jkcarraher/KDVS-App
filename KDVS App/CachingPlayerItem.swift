...
open class CachingPlayerItem: AVPlayerItem {
    ...
    class ResourceLoaderDelegate: NSObject, AVAssetResourceLoaderDelegate, URLSessionDelegate, URLSessionDataDelegate, URLSessionTaskDelegate {
        ...
        func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
            ...
            if playingFromData {
                print("Playing from Data - mimeType: \(mimeType ?? "unknown")")
                // Nothing to load.
                
            } else if session == nil {
                print("Start data request with URL: \(owner?.url.absoluteString ?? "nil")")
        ...
        func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
            print("Received response with mimeType: \(response.mimeType ?? "unknown"), contentLength: \(response.expectedContentLength)")
        ...
        func fillInContentInformationRequest(_ contentInformationRequest: AVAssetResourceLoadingContentInformationRequest?) {
            if playingFromData {
                ...
                print("Filling content information request from Data - mimeType: \(mimeType ?? "unknown"), contentLength: \(mediaData?.count ?? 0)")
                return
            }
            ...
            if let asset = owner?.asset as? AVURLAsset, let duration = asset.duration as? CMTime {
                    // Check if the duration is available and greater than 0
                    if duration.seconds > 0 {
                        // Set the content length based on the duration
                        contentInformationRequest?.contentLength = Int64(duration.seconds * 1000) // Convert duration to milliseconds
                    }
                }
            ...
            contentInformationRequest?.isByteRangeAccessSupported = true
            print("Filling content information request from URL - mimeType: \(responseUnwrapped.mimeType ?? "unknown"), contentLength: \(responseUnwrapped.expectedContentLength)")
